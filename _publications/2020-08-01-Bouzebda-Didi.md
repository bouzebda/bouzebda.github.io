...
an $\mathbb{R}^{d}\times\mathbb{R}^{q}$-valued strictly stationary and

ergodic continuous time process, where the regression function is given by 

$m(\mathbf{x},\psi) = \mathbb{E}(\psi(\mathbf{ Y}) \mid \mathbf{ X} = \mathbf{ x}))$,  

for a measurable function
$\psi : \mathbb{R}^{q} \rightarrow \mathbb{R}$.  

We focus on the estimation of the location 

$m(\mathbf{x},\psi) = \mathbb{E}(\psi(\mathbf{ Y}) \mid \mathbf{ X} = \mathbf{ x}))$


$\mathbf{\Theta}$ (mode) of a unique maximum of $m(\cdot, \psi)$ by the location  

$\widehat{\mathbf{\Theta}}_{\rm T}$ 

of a maximum of the Nadaraya-Watson kernel estimator $\widehat{m}_{\rm T}(\cdot, \psi)$


for the curve $m(\cdot, \psi)$. 

Within this context, we obtain the consistency with rate  and the asymptotic normality results for 
